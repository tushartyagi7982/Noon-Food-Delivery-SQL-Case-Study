...
select * from orders        
--1. Find Top 1 Outlets by cuisine type without using limit and top function
with cte as(
...
from cte) a
where rn=1
--2. Find daily new customer count from the launch date (everyday how many new customers are we acquiring)
with cte as (
...
--3. Count of all the users who were acquired in Jan 2025 and only placed one order in Jan and did not placed any other order.
...
--4. List all the customers with no order in the last 7 days(today=2025-03-31) but were acquired one month ago with their first order on promo.
with cte as (
select customer_code , min(placed_at) as first_order_date, max(placed_at) as latest_order_date
from orders
group by customer_code)
select cte.*, orders.promo_code_name as first_order_promo from cte inner join orders on cte.customer_code=orders.customer_code and
...
from orders)
select * from cte where order_number%3=0 and cast(placed_at as date)='2025-03-31'
